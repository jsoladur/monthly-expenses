...
3. Neither doc covers it → do NOT invent frameworks, libraries, vendors, or behavior. Ask the Product Owner (PRD scope rule, §1).
4. Docs vs code → docs win.

## What UC-INDEX.md means

The PRD is split into 13 implementation slices, `UC-00` … `UC-12`, so the app can be built in steps. `UC-INDEX.md` is the entry point that ties them together:
...
- **Key architectural fact:** UC-00 creates the ENTIRE database in one migration (from `database.dbml`). Slices UC-01…UC-12 contain NO schema work — they only use the existing tables. Do not add tables or columns when implementing a slice.

## Standard work loop (agents)

1. Read `IMPLEMENTATION-STATUS.md` → pick the first PENDING slice whose dependencies are all DONE.
2. Read the slice file, then the PRD and architecture sections it references.
