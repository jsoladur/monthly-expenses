...
| `database/prod/` (gitignored) | Local tenant dump for restoring a private DB. Not committed. | Local restore only. |
...
- **Schema:** table names are singular; the users table is `app_user` because `user` is reserved in PostgreSQL. Only UC-14 adds a table (`annual`). UC-16 (Search) and UC-17 (actual name autocomplete) add none.
