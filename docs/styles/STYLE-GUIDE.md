...
- **Actual name autocomplete (UC-17):** inline list under `#new-actual-name` on the add-actual form (in flow, not overlay). Max **5** options, each `h-11` (44px), `text-sm`, ink on `--card`, active/keyboard row **sky-tint**. No search icon, no empty-state row, no month/amount captions. Native `<datalist>` forbidden. Same tokens in dark mode (`--card` / `--secondary`).
- **Pass to upcoming month (UC-18):** bottom `Drawer` from an estimated-card icon. Options are later **created** months of the current year only (`h-11`, locale month name). Selected row: **sky-tint** fill + teal (`--estimated`) left rail. Confirm is navy primary. No calendar grid, no native `<select>`, no empty “missing month” rows. Hidden on History and other years.
- **Excel export (UC-19):** Home header **Export** opens a bottom `Drawer`. Three mode cards (All / year / specific months). Year and month lists are **descending checkboxes**, only periods that already exist, `h-11`, sky-tint + navy left rail when selected. Download is navy primary. No date widget, no native `<select>`, no 12-month grid. No brand gradient on this control.
