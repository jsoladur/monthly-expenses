...
> **PRD status:** Merged at high level into `docs/prds/GLOBAL.md` as **UC-20** / C19 / §6.8 / §7.9 / §10 screen 11 / §13 (PRD **UC-14** remains home / month navigation). This file is the detailed implementation slice.
...
A per-user catalog of **yearly recurring expenses** (e.g., annual insurance, vehicle tax) plus **reminders in the month workspace**: when the open month's month-number matches an annual's charge month — September of ANY year — show one reminder per matching annual prompting the user to MANUALLY add an estimated/committed line. Annuals never auto-create anything (consistent with PRD C6/C12 philosophy).
...
- E2E: create annual "Home insurance" (charge_month = 9, domiciliado = true, amount = 250.00) → open/create September 2026 → reminder card visible with amount → open October 2026 → not visible → open September 2027 → visible again (any year).
